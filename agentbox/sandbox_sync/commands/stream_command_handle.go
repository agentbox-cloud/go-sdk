...
func (h *streamCommandHandle) handleEndEvent(end *connect.EndEvent) {
	...
	// Use exit code from EndEvent, defaulting to 0 if not set (matching protobuf semantics)
	exitCode := int(end.ExitCode)
	// In protobuf, if exit_code is omitted, it defaults to 0 (proto3 scalar default)
	// But if it's explicitly set to 0, it will be 0
